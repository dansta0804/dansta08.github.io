...
doctor_server <- function(input, output, session) {
  ...
  observe({
    ...
    output$download1 <- downloadHandler(
      filename = function() {paste("Peak_counts", "png", sep = ".")},
      content = function(file){ggsave(file, plot1(), device = "png")}
    )
    ...
    output$download2 <- downloadHandler(
      filename = function() {paste("Peak_distributions_chr", "png", sep = ".")},
      content = function(file){ggsave(file, plot2(), device = "png")}
    )
    ...
    output$download3 <- downloadHandler(
      filename = function() {paste("Sample_similarity", "png", sep = ".")},
      content = function(file){ggsave(file, plot3(), device = "png")}
    )
    ...
    output$download4 <- downloadHandler(
      filename = function() {paste("Genomic_distribution", "png", sep = ".")},
      content = function(file){ggsave(file, plot4(), device = "png")}
    )
    ...
    output$download8 <- downloadHandler(
      filename = function() {paste("PWM_atitikimai", "png", sep = ".")},
      content = function(file){ggsave(file, plot8(), device = "png")}
    )

    # GENOMIC DATA ANALYSIS - GO ANALYSIS (TABLES):
    output$table1 <-
    ...
    # GENOMIC DATA ANALYSIS - GO ANALYSIS (ACYCLIC GRAPH):
    output$plot9 <-
    ...
    # GENOMIC DATA ANALYSIS - GO ANALYSIS (TREE PLOT):
    output$plot12 <-
